...
DEFAULT_TWIC_PGN = Path(r"C:\Users\GOWTHAM\Downloads\July\opponent game research\Twic Last 3 year.pgn")
...
def run_extraction(job_id, config):
    try:
        ...
        twic_path = Path(config["twic_path"])
        ...
        try:
            with final_pgn.open("wb") as final_fh, chessresults_pgn.open("wb") as cr_fh:
                ...
                add_log(job_id, "Scanning TWIC PGN...")
                scanned = 0
                for game_lines in iter_pgn_games(twic_path):
                    scanned += 1
                    process_game(game_lines, "TWIC", final_fh)
                    if scanned % 100000 == 0:
                        add_log(job_id, f"TWIC: scanned {scanned:,} games, {len(seen_final)} unique games.")
                        update_job(job_id, games=len(seen_final))
        finally:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/api/defaults":
            self.send_json(
                {
                    "twic_path": str(DEFAULT_TWIC_PGN),
